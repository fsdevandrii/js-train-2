//Задача 1: Напишіть функцію, яка буде виконувати js код який передали в функцію через аргумент
// в форматі тексту
...
function calculateExpression(expression) {
  ...
  if (typeof expression === "string") {
    try {
      return eval(expression); // Виконуємо вираз і повертаємо результат
    } catch (error) {
      return "Error: " + error.message; // Повертаємо помилку, якщо вираз не можна виконати
    }
  } else {
...
function getNumberFromString(s) {
  ...
  // Використовуємо parseFloat для спроби отримати число з рядка
  const number = parseFloat(s);

  // Перевіряємо, чи вдалося отримати число і чи воно не є NaN
  if (!isNaN(number)) {
    return number;
  } else {
...
function getIntegerFromString(s) {
  ...
  // Використовуємо parseInt для спроби отримати ціле число з рядка
